...
A **scenario tool** wraps an entire subscenario as an agent tool. This is for multi-step logic that can't fit in a single module tool.
...
2. **Trigger: `Scenarios > Start scenario`** — receives the inputs the agent passes in
3. **End with `Scenarios > Return outputs`** — sends results back to the agent
4. **Named input/output fields** — defined in the subscenario creation window; structure must be accurate or data passes with errors

### In the parent blueprint

The blueprint JSON structure for scenario tools is auto-generated by the Make UI when you add a scenario tool to an agent. **Do not hand-write scenario tool JSON** — the structure differs from module tools and cannot be reliably reconstructed without a live reference.

**To get the correct blueprint structure:** configure the scenario tool in the Make UI, then call `extract_blueprint_components` or `scenarios_get` on that scenario to extract the exact JSON. Use that as your template.

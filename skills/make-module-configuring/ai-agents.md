...
**Critical:** The AI agent module outputs its result as `{{N.response}}`, not `{{N.output}}`.

- Correct: `{{2.response}}`
- Wrong: `{{2.output}}` — this field is always empty

This catches everyone because Make's convention for many modules uses `output`. For `ai-local-agent:RunLocalAIAgent`, always use `response`.

Additional output fields:
- `{{N.metadata}}` — execution steps and token usage summary
- `{{N.reasoning}}` — reasoning breakdown (only on reasoning models)

## Scenario Tools: Requirements and Blueprint Structure

A **scenario tool** wraps an entire subscenario as an agent tool using the `scenario-service:CallSubscenario` module inside the tool's `flow`. This is for multi-step logic that can't fit in a single module tool.

### Subscenario requirements (must all be true before adding as a tool)

1. **Active and scheduled "On demand"** — set in the subscenario's scheduling settings
2. **Trigger: `scenario-service:StartSubscenario`** — "Start scenario" — receives the inputs the agent passes in
3. **End with `scenario-service:ReturnData`** — "Return output" — sends results back to the agent
4. **Named input/output fields** — defined accurately in the subscenario setup; field name mismatches cause silent data loss

### Module IDs (scenario-service@2)

| Module | ID | Role |
|---|---|---|
| Call a scenario | `scenario-service:CallSubscenario` | Goes inside the tool `flow` |
| Start scenario | `scenario-service:StartSubscenario` | Trigger in the subscenario |
| Return output | `scenario-service:ReturnData` | Final module in the subscenario |

### `CallSubscenario` parameters

| Parameter | Type | Notes |
|---|---|---|
| `scenario` | `scenario` | The subscenario to call — resolved via RPC, references scenario by ID |
| `shouldWaitForExecutionEnd` | boolean | `true` = sync (wait for output); `false` = async (fire and forget). Default: `true` |
...
- **`{{N.response}}` not `{{N.output}}`** — the agent's text result is always in `response`. `output` is permanently empty.
...
- **Scenario tool subscenario must be on-demand + active.** If it shows "Inactive" or has a scheduled trigger, it cannot be called as a tool.
